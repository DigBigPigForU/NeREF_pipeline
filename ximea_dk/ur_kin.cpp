...
namespace ur_kinematics {
	...
	{
        double dis[8];
        for (int i = 0; i < 8; i++)//something wrong  8 cannot be replaced by num
		{
			double temp[6] = { q_ori[i * 6 + 0],q_ori[i * 6 + 1],q_ori[i * 6 + 2],q_ori[i * 6 + 3],q_ori[i * 6 + 4],q_ori[i * 6 + 5] };
			
			dis[i] = distance_(temp, q_cri);
		}
        int i = std::min_element(dis, dis + sizeof(dis)/sizeof(dis[0])) - dis;
        std::cout<<"i:"<<i<<std::endl;
        double res[6] = { q_ori[i * 6 + 0],q_ori[i * 6 + 1],q_ori[i * 6 + 2],q_ori[i * 6 + 3],q_ori[i * 6 + 4],q_ori[i * 6 + 5] };
        std::cout<<"res:"<<res[0]<<","<<res[1]<<","<<res[2]<<","<<res[3]<<","<<res[4]<<","<<res[5]<<std::endl;
        for (int x = 0; x < 6; x++)
        {
            q_res[x] = res[x];
        }
        std::cout<<"q_res:"<<q_res[0]<<","<<q_res[1]<<","<<q_res[2]<<","<<q_res[3]<<","<<q_res[4]<<","<<q_res[5]<<std::endl;
//        q_res = res;
	}
